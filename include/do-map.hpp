...
private:
    ...
    int                                   num_map_;
    int                                   num_reducers_;
    queue<pair<string,string>>            queue_jobs_;
    ...
    thread_local static vector<pair<UserReduce::Key, UserReduce::Value>>      store;
    thread_local static std::unordered_map<UserReduce::Key, vred_val>         key_vvalues;
    thread_local static vector<pair<UserReduce::Key, vred_val>>               ready_send;

    void Send(vector<pair<UserReduce::Key, UserReduce::Value>>&);

    void map_worker(int, UserMap);
